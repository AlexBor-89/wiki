...
Миграция Red Hat Enterprise Linux 6.10 (RHEL) на CentOS 6.10
Проверим текущую установленную версию ОС:
...
**Импортируем ключи CentOS**
```bash
cd /etc/pki/rpm-gpg/
wget https://www.centos.org/keys/RPM-GPG-KEY-CentOS-Official --no-check-certificate
gpg --quiet --with-fingerprint RPM-GPG-KEY-CentOS-Official
```

**Меняем репозитории с Redhat’s на CentOS**
```bash
...
[CentOS_6_UPDATE_OLD]
name=Cen8tOS-6
baseurl=https://buildlogs.centos.org/centos/6/upg/old/$basearch
gpgcheck=0
enabled=1
...
[CentOS_6_UPDATE]
name=Cen8tOS-6
baseurl=https://buildlogs.centos.org/centos/6/upg/$basearch
gpgcheck=0
enabled=1
gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-Official
...
mkdir /root/migrate
cd ~/migrate
wget https://vault.centos.org/6.10/os/x86_64/Packages/centos-indexhtml-6-2.el6.centos.noarch.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/Packages/centos-release-6-10.el6.centos.12.3.x86_64.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/Packages/python-urlgrabber-3.9.1-11.el6.noarch.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/Packages/yum-3.2.29-81.el6.centos.noarch.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/Packages/yum-plugin-fastestmirror-1.1.30-41.el6.noarch.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/Packages/yum-utils-1.1.30-41.el6.noarch.rpm --no-check-certificate
wget https://vault.centos.org/6.10/os/x86_64/RPM-GPG-KEY-CentOS-6 --no-check-certificate
```

> Или с локального msk-wiki, но нужно установить сертификат:
> ```bash
> wget https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:centos-indexhtml-6-2.el6.centos.noarch.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:centos-release-6-10.el6.centos.12.3.x86_64.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:python-urlgrabber-3.9.1-11.el6.noarch.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:yum-3.2.29-81.el6.centos.noarch.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:yum-plugin-fastestmirror-1.1.30-41.el6.noarch.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:yum-utils-1.1.30-41.el6.noarch.rpm \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:convert-redhat6-centos6.sh \
> https://msk-wiki/lib/exe/fetch.php?media=sap:linux:migrate_rhel_to_centos:RPM-GPG-KEY-CentOS-6 --no-check-certificate
> ```
<!-- {blockquote:.is-info} -->

**Выполняем миграцию**
Если файл convert-redhat6-centos6.sh не скачивали, то его нужно создать вручную:
```bash
nano /root/migrate/convert-redhat6-centos6.sh
```
...
declare target_version="6.10" # target Centos version
declare centos_key="RPM-GPG-KEY-CentOS-6"
declare centos_release_rpm="centos-release-6-10.el6.centos.12.3.x86_64.rpm"
...
declare yum_rpm="yum-3.2.29-81.el6.centos.noarch.rpm"
declare yum_fastestmirror_rpm="yum-plugin-fastestmirror-1.1.30-41.el6.noarch.rpm"
declare yum_utils_rpm="yum-utils-1.1.30-41.el6.noarch.rpm"
...
#
  if [[ ! -f ./$centos_key ]]; then
    (( ! _DRYRUN )) && wget https://vault.centos.org/$target_version/os/$arch/$centos_key || true
  fi
  [[ ! -f ./$centos_key ]] && die "could not download $centos_key"||print "$centos_key downloaded"
#
  if [[ ! -f ./$python_urlgrabber ]];then
    (( ! _DRYRUN )) &&  wget https://vault.centos.org/$target_version/os/$arch/Packages/$python_urlgrabber||true
  fi
...
#
  if [[ ! -f ./$centos_release_rpm ]]; then
    (( ! _DRYRUN )) && wget https://vault.centos.org/$target_version/os/$arch/Packages/$centos_release_rpm ||true
  fi
#
...
#
  if [[ ! -f ./$centos_indexhtml_rpm ]]; then
    (( ! _DRYRUN )) && wget https://vault.centos.org/$target_version/os/$arch/Packages/$centos_indexhtml_rpm||true
  fi
  [[ ! -f ./$centos_indexhtml_rpm ]] && die "could not download $centos_indexhtml_rpm"||print "$centos_indexhtml_rpm downloaded"
#
  if [[ ! -f ./$yum_rpm ]]; then
    (( ! _DRYRUN )) && wget https://vault.centos.org/$target_version/os/$arch/Packages/$yum_rpm||true
  fi
  [[ ! -f ./$yum_rpm ]] && die "could not download $yum_rpm"||print "$yum_rpm downloaded"
#
  if [[ ! -f ./$yum_fastestmirror_rpm ]]; then
    (( ! _DRYRUN )) && wget https://vault.centos.org/$target_version/os/$arch/Packages/$yum_fastestmirror_rpm||true
  fi
...
Запускаем миграцию:
```bash
sh ./convert-redhat6-centos6.sh CONVERT
```
...
- https://ftp.redhat.com/redhat/linux/enterprise/6Server/en/os/SRPMS/
- https://www.repo.cloudlinux.com/cloudlinux/migrate/release-files/centos/6/x86_64/